...
from dyna_route.datasets.dyna_route_window_dataset import DynaRouteWindowDataset
from dyna_route.models.modeling_dyna_route import DynaRouteForPrediction, DynaRouteConfig
from dyna_route.trainer.hf_trainer import DynaRouteTrainingArguments, DynaRouteTrainer
from dyna_route.utils.dist_util import get_world_size
from dyna_route.utils.log_util import logger, log_in_local_rank_0


class DynaRouteRunner:
    def __init__(
            self,
            model_path: str = None,
            output_path: str = 'logs/dyna_route',
            seed: int = 9899
    ):
    ...
    def load_model(self, model_path: str = None, from_scatch: bool = False, **kwargs):
        ...
        dyna_route_config = {}
        requested_use_dyna_route = kwargs.pop('use_dyna_route', None)
        dyna_route_kwargs = {}
        for key in list(kwargs.keys()):
            if key.startswith('dyna_route_'):
                dyna_route_kwargs[key] = kwargs.pop(key)
        if requested_use_dyna_route:
            dyna_route_config['use_dyna_route'] = True
            for key, value in dyna_route_kwargs.items():
                if value is not None:
                    dyna_route_config[key] = value

        if from_scatch:
            config = DynaRouteConfig.from_pretrained(model_path, _attn_implementation=attn)
            for key, value in dyna_route_config.items():
                setattr(config, key, value)
            if dyna_route_config:
                config.dyna_route_code_dim = config.dyna_route_code_dim or config.hidden_size
                config.dyna_route_residual_dim = config.dyna_route_residual_dim or config.hidden_size
                config.dyna_route_token_size = config.dyna_route_code_dim + config.dyna_route_residual_dim
                config.dyna_route_router_dim = config.dyna_route_router_dim or config.hidden_size
            model = DynaRouteForPrediction(config)
        else:
            if dyna_route_config:
                config = DynaRouteConfig.from_pretrained(model_path, _attn_implementation=attn)
                for key, value in dyna_route_config.items():
                ...
                model = DynaRouteForPrediction.from_pretrained(model_path, **kwargs)
        return model
    ...
    def train_model(self, from_scratch: bool = False, **kwargs):
        ...
        training_argument_kwargs = dict(
            ...
            dataloader_num_workers=2 if train_config.get('dataloader_num_workers') is None else train_config.get('dataloader_num_workers'),
        ...
        training_args_signature = inspect.signature(DynaRouteTrainingArguments.__init__)
        if 'evaluation_strategy' in training_args_signature.parameters:
            training_argument_kwargs['evaluation_strategy'] = train_config.get("evaluation_strategy", 'no')
        else:
            training_argument_kwargs['eval_strategy'] = train_config.get("evaluation_strategy", 'no')
        training_args = DynaRouteTrainingArguments(**training_argument_kwargs)
        ...
        if model_path is not None:
            model = self.load_model(
                ...
                use_dyna_route=train_config.get('use_dyna_route', False),
                dyna_route_codebook_size=train_config.get('dyna_route_codebook_size'),
                dyna_route_code_dim=train_config.get('dyna_route_code_dim'),
                dyna_route_residual_dim=train_config.get('dyna_route_residual_dim'),
                dyna_route_router_dim=train_config.get('dyna_route_router_dim'),
                dyna_route_condition_decoding=train_config.get('dyna_route_condition_decoding'),
                dyna_route_consistency_loss_factor=train_config.get('dyna_route_consistency_loss_factor'),
                dyna_route_separation_loss_factor=train_config.get('dyna_route_separation_loss_factor'),
                dyna_route_routing_loss_factor=train_config.get('dyna_route_routing_loss_factor'),
            )
    ...
    def get_train_dataset(self, data_path, max_length, stride, normalization_method):
        ...
        dataset = DynaRouteDataset(data_path, normalization_method=normalization_method)
        log_in_local_rank_0('Processing dataset to fixed-size sub-sequences...')
        window_dataset = DynaRouteWindowDataset(dataset, context_length=max_length, prediction_length=0, stride=stride, shuffle=False)
